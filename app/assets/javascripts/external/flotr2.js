...
/*2014 modification by Pi Trem
 * added type rectangle draw function 
 * added xaxis option ( directionreversed axis  from right to left )
 * */
...
Flotr.defaultOptions = {
  ...
  parseFloat: false,        // => whether to preprocess data for floats (ie. if input is string)
...
function getRowLabel(value){
  if (this.options.spreadsheetJdx.tickFormatter){
    //TODO maybe pass the xaxis formatter to the custom tick formatter as an opt-out?
    return this.options.spreadsheetJdx.tickFormatter(value);
  }
  else {
    var t = _.find(this.axes.x.ticks, function(t){return t.v == value;});
    if (t) {
...
Flotr.addPlugin('spreadsheetJdx', {
	  options: {
	    show: false,           // => show the data grid using two tabs
	    tabGraphLabel: 'Graph',
	  ...
	  callbacks: {
	    'flotr:afterconstruct': function(){
	      ...
	      if (!this.options.spreadsheetJdx.show) return;
	      
	      var ss = this.spreadsheetJdx,
	        container = D.node('<div class="flotr-tabs-group" style="position:absolute;left:0px;width:'+this.canvasWidth+'px"></div>'),
	  ...
	  constructDataGrid: function(){
	    // If the data grid has already been built, nothing to do here
	    if (this.spreadsheetJdx.datagrid) return this.spreadsheetJdx.datagrid;
	    
	    var s = this.series,
	        datagrid = this.spreadsheetJdx.loadDataGrid(),
	        colgroup = ['<colgroup><col />'],
	        buttonDownload, buttonSelect, t;
	    ...
	    */
	    ...
	    this.
	      observe(buttonDownload, 'click', _.bind(this.spreadsheetJdx.downloadCSV, this)).
	      observe(buttonSelect, 'click', _.bind(this.spreadsheetJdx.selectAllData, this));
	    ...
	    this.spreadsheetJdx.containerdata = container;

	    return t;
	  },
	  
	  loadldrGrid: function(){
		    if (this.seriesldr) return this.seriesldr;
		    //
		    var s =  this.options.spreadsheetJdx.ldrS || this.options.spreadsheetJdx.ldrSeries,
		        rows = [],
		       // i = 0,        //row
		        j = 0;       //col
		    ...
		    **/
		    ...
		    // The data grid is sorted by x value
		  //  this.seriesldr = _.sortBy(rows, function(row, x){
		   //   return parseInt(x, 10);
		   // });
		    this.seriesldr = rows;
	  ...
	  constructLdrGrid: function(){
		    // If the data grid has already been built, nothing to do here
		   if (this.spreadsheetJdx.ldrgrid) return this.spreadsheetJdx.ldrgrid;
...
//		        buttonDownload, buttonSelect, t;
		  ...
		  //  });
		    ...
		    this.spreadsheetJdx.containerldr = container;
...
//		    }
	  ...
	  showTab: function(tabName){
	    ...
	    this.spreadsheetJdx.activeTab = tabName;
	  },
	  /**
	   * Selects the data table in the DOM for copy/paste
	   */
	  selectAllData: function(){
	    if (this.spreadsheetJdx.tabs) {
	      var selection, range, doc, win, node = this.spreadsheetJdx.constructDataGrid();

	      this.spreadsheetJdx.showTab('data');
	  ...
	  selectAllLdr: function(){
		    if (this.spreadsheetJdx.tabs) {
		      var selection, range, doc, win, node = this.spreadsheetJdx.constructLdrGrid();

		      this.spreadsheetJdx.showTab('ldr');
	  ...
	  downloadCSV: function(){
	    var csv = '',
	        series = this.series,
	        options = this.options,
	        dg = this.spreadsheetJdx.loadDataGrid(),
	        separator = encodeURIComponent(options.spreadsheetJdx.csvFileSeparator);
	    
	    if (options.spreadsheetJdx.decimalSeparator === options.spreadsheetJdx.csvFileSeparator) {
	      throw "The decimal separator is the same as the column separator ("+options.spreadsheetJdx.decimalSeparator+")";
	    }
	    ...
	    csv += _.reduce(dg, function(memo, row){
	      ...
	      rowLabel = '"'+(rowLabel+'').replace(/\"/g, '\\"')+'"';
	      var numbers = row.slice(1).join(separator);
	      if (options.spreadsheetJdx.decimalSeparator !== '.') {
	        numbers = numbers.replace(/\./g, options.spreadsheetJdx.decimalSeparator);
	      }
	      return memo + rowLabel+separator+numbers+"%0D%0A"; // \t and \r\n
	    }, '', this);
	  ...
	  downloadLdr: function(){
//		    var csv = '',
//		        series = this.options.spreadsheetJdx.ldrSeries,
//		        options = this.options,
//		        dg = this.spreadsheetJdx.loadldrGrid(),
//		        separator = encodeURIComponent(options.spreadsheetJdx.csvFileSeparator);
//		    
//		    if (options.spreadsheetJdx.decimalSeparator === options.spreadsheetJdx.csvFileSeparator) {
//		      throw "The decimal separator is the same as the column separator ("+options.spreadsheetJdx.decimalSeparator+")";
